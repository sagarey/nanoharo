...
<task type="auto">
  <name>Task 1: 更新 README.md 和 CLAUDE.md 反映去容器化架构</name>
...
**CLAUDE.md 更新**（同一个 task 处理）：

CLAUDE.md 是提供给 Claude Code 的项目指引，当前有多处过时描述：

1. **Quick Context**：改为 `Single Node.js process that connects to WhatsApp, runs Claude Agent SDK in-process. Each group has isolated directory (groups/{name}/) and memory (CLAUDE_HOME).`

2. **Key Files 表格**：
   - 删除 `src/container-runner.ts` 行
   - 删除 `container/skills/agent-browser.md` 行（container/ 目录已删除）
   - 新增 `src/agent-runner.ts | In-process Claude Agent SDK runner (V2 sessions)`
   - 新增 `src/group-queue.ts | Per-group message queue with SDKSession handles`

3. **Development 章节**：
   - 删除 `./container/build.sh # Rebuild agent container` 这行

4. **Container Build Cache 章节**：整个章节删除（container/ 目录已删除，不再需要 buildkit 注意事项）

5. **GSD Tools 路径**：路径 `/Users/levi/Projects/nanoharo/` 是本地硬编码路径，改为相对路径：`node "./.claude/get-shit-done/bin/gsd-tools.cjs" <command>` 或 `node "$PWD/.claude/get-shit-done/bin/gsd-tools.cjs" <command>`
  </action>
  <verify>
    <automated>grep -n "container sandbox\|Apple Container\|Docker.*container.*agent\|container-runner\|container/skills" /workspaces/nanoharo/README.md /workspaces/nanoharo/CLAUDE.md; echo "exit: $?"</automated>
  </verify>
  <done>
    README.md 不再有误导性容器架构描述；CLAUDE.md Quick Context、Key Files、Development 均反映去容器化后的实际状态。
  </done>
</task>
